...
// Hamburg Mene setting on screen size change start
window.addEventListener('resize', function () {
    if (window.matchMedia('(min-width: 640px)').matches) {
        document.getElementById('hamburg_checkbox').checked = false;
    }
}, true);


// Hamburg Mene setting on screen size change end



// this code is for Food Menu Slide show start
...
// this code is for Food Menu Slide show end



// this code is for Discover Section start
...
function showSlides() {
  let i;
  let text_slides = document.getElementsByClassName("disc_text");
  let slide_image = document.querySelectorAll(".disc_images_div img");
...
// for text slider
  ...
  for (i = 0; i < text_slides.length; i++) {
    text_slides[i].style.display = "none";
...
// for image slider
  ...
  slideIndex++;
  if (slideIndex > text_slides.length) {slideIndex = 1}
  if (slideIndex > slide_image.length) {slideIndex = 1}

  text_slides[slideIndex-1].style.display = "block";
  slide_image[slideIndex-1].style.display = "block";
  dot[slideIndex-1].classList.add("active");
  setTimeout(showSlides, 6000); // Change image every 2 seconds
}

// this code is for Discover Section end